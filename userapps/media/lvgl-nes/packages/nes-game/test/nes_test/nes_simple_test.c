...
#if RT_NES_PLATFORM_POSIX
    pthread_t tid1;
    int ret = pthread_create(&tid1, NULL, &nes_run, (void *)my_nes);
    if (ret != 0)
    {
...
#elif RT_NES_PLATFORM_FREERTOS
    taskENTER_CRITICAL();
    xTaskCreate(nes_run, "nes_run", 2048, (void *)my_nes, (tskIDLE_PRIORITY + 3), (TaskHandle_t *)NULL);
    printf("Free heap: %d bytes\n\r", xPortGetFreeHeapSize());
    taskEXIT_CRITICAL();
#elif RT_NES_PLATFORM_RTTHREAD
    rt_thread_t thread = NULL;
    thread = rt_thread_create("nes", nes_run, (void *)my_nes, 4096, 22, 10);
    if (thread == NULL)
    {
        return -1;
    }
...
static void init_my_nes_front_end(lv_obj_t *parent)
{
    ...
    lv_obj_set_size(btnm_opt, LV_PCT(25), LV_PCT(25));
    ...
    lv_obj_t *file_explorer = file_explorer_create(tab2);
...
#if LV_USE_FS_WIN32
    file_explorer_open_dir(file_explorer, "D:/workspace_rb/");
#else
    file_explorer_open_dir(file_explorer, "/bin");
#endif
    ...
    nes_set_state(parent, RT_NES_STATE_MENU);
    nes_set_key(parent, RT_NES_KEY_MENU, RT_NES_KEY_STATE_RELEASED);
    nes_set_lock(parent);
}
...
static void ctrl_btnm_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_VALUE_CHANGED)
    {
        ...
        if (strcmp(txt, LV_SYMBOL_UP) == 0)
            nes_set_key(nes, RT_NES_KEY_UP, RT_NES_KEY_STATE_PRESSED);
        else if (strcmp(txt, LV_SYMBOL_DOWN) == 0)
            nes_set_key(nes, RT_NES_KEY_DOWN, RT_NES_KEY_STATE_PRESSED);
        else if (strcmp(txt, LV_SYMBOL_LEFT) == 0)
            nes_set_key(nes, RT_NES_KEY_LEFT, RT_NES_KEY_STATE_PRESSED);
        else if (strcmp(txt, LV_SYMBOL_RIGHT) == 0)
            nes_set_key(nes, RT_NES_KEY_RIGHT, RT_NES_KEY_STATE_PRESSED);
        else if (strcmp(txt, "A") == 0)
            nes_set_key(nes, RT_NES_KEY_A, RT_NES_KEY_STATE_PRESSED);
        else if (strcmp(txt, "B") == 0)
            nes_set_key(nes, RT_NES_KEY_B, RT_NES_KEY_STATE_PRESSED);
        else if (strcmp(txt, "START") == 0)
            nes_set_key(nes, RT_NES_KEY_START, RT_NES_KEY_STATE_PRESSED);
        else if (strcmp(txt, "SELECT") == 0)
            nes_set_key(nes, RT_NES_KEY_SELECT, RT_NES_KEY_STATE_PRESSED);
    ...
    else if (code == LV_EVENT_RELEASED)
    {
        ...
        if (strcmp(txt, LV_SYMBOL_UP) == 0)
            nes_set_key(nes, RT_NES_KEY_UP, RT_NES_KEY_STATE_RELEASED);
        else if (strcmp(txt, LV_SYMBOL_DOWN) == 0)
            nes_set_key(nes, RT_NES_KEY_DOWN, RT_NES_KEY_STATE_RELEASED);
        else if (strcmp(txt, LV_SYMBOL_LEFT) == 0)
            nes_set_key(nes, RT_NES_KEY_LEFT, RT_NES_KEY_STATE_RELEASED);
        else if (strcmp(txt, LV_SYMBOL_RIGHT) == 0)
            nes_set_key(nes, RT_NES_KEY_RIGHT, RT_NES_KEY_STATE_RELEASED);
        else if (strcmp(txt, "A") == 0)
            nes_set_key(nes, RT_NES_KEY_A, RT_NES_KEY_STATE_RELEASED);
        else if (strcmp(txt, "B") == 0)
            nes_set_key(nes, RT_NES_KEY_B, RT_NES_KEY_STATE_RELEASED);
        else if (strcmp(txt, "START") == 0)
            nes_set_key(nes, RT_NES_KEY_START, RT_NES_KEY_STATE_RELEASED);
        else if (strcmp(txt, "SELECT") == 0)
            nes_set_key(nes, RT_NES_KEY_SELECT, RT_NES_KEY_STATE_RELEASED);
        else if (strcmp(txt, LV_SYMBOL_LIST) == 0)
        {
            lv_obj_add_flag(lv_layer_top(), LV_OBJ_FLAG_CLICKABLE);
            lv_obj_clear_flag(lv_layer_top(), LV_OBJ_FLAG_HIDDEN);
            nes_set_key(nes, RT_NES_KEY_MENU, RT_NES_KEY_STATE_RELEASED);
        }
        else
        {
            nes_set_key(nes, RT_NES_KEY_CLEAN, RT_NES_KEY_STATE_RELEASED);
        }
        // LV_LOG_USER("KE UP");
    }
}
...
static void file_explorer_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_VALUE_CHANGED)
    {
        char *path = file_explorer_get_cur_path(file_explorer);
        char *fn = file_explorer_get_sel_fn(file_explorer);
        uint16_t path_len = strlen(path);
        uint16_t fn_len = strlen(fn);

        if ((path_len + fn_len) <= RT_FILE_EXPLORER_PATH_MAX_LEN)
        {
            char sel_fn[RT_FILE_EXPLORER_PATH_MAX_LEN];
            ...
            nes_set_state(nes, RT_NES_STATE_NEW_GAME);
            nes_set_fn(nes, sel_fn);
...
static void slider_event_cb(lv_event_t *e)
{
    ...
    {
        nes_set_zoom(my_nes, lv_slider_get_value(slider));
    }
    else if (strcmp(txt, "Velocity") == 0)
    {
        nes_set_speed(my_nes, lv_slider_get_value(slider));
    }
}
