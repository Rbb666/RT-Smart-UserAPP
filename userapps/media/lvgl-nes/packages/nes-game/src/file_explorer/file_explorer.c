 ...
 *      DEFINES
 *********************/
#define MY_CLASS &file_explorer_class
 ...
 *  STATIC PROTOTYPES
 **********************/
static void file_explorer_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj);
static void file_explorer_destructor(const lv_obj_class_t * class_p, lv_obj_t * obj);
...
const lv_obj_class_t file_explorer_class = {
    .constructor_cb = file_explorer_constructor,
    .destructor_cb  = file_explorer_destructor,
...
void file_explorer_set_quick_access_path(lv_obj_t * obj, file_explorer_dir_t dir, char * path)
{
    ...
    switch (dir)
    {
        case RT_EXPLORER_HOME_DIR:
            dir_str = &(explorer->home_dir);
            break;
        case RT_EXPLORER_MUSIC_DIR:
            dir_str = &(explorer->music_dir);
            break;
        case RT_EXPLORER_PICTURES_DIR:
            dir_str = &(explorer->pictures_dir);
            break;
        case RT_EXPLORER_VIDEO_DIR:
            dir_str = &(explorer->video_dir);
            break;
        case RT_EXPLORER_DOCS_DIR:
            dir_str = &(explorer->docs_dir);
            break;
        //case RT_EXPLORER_MNT_DIR:
        //    dir_str = &(explorer->home_dir);
        //    break;
        //case RT_EXPLORER_FS_DIR:
        //    dir_str = &(explorer->home_dir);
        //    break;
...
lv_obj_t * file_explorer_get_head(lv_obj_t * obj)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
...
static void brower_file_event_handler(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_VALUE_CHANGED) {
        ...
        char * file_name[RT_FILE_EXPLORER_PATH_MAX_LEN];
        char * str_fn = NULL;
        uint16_t row;
        uint16_t col;
...
static void show_dir(lv_obj_t * obj, char * path)
{
    ...
    char fn[RT_FILE_EXPLORER_PATH_MAX_LEN];
    uint16_t index = 0;
    lv_fs_dir_t dir;
    lv_fs_res_t res;
...
#endif  /*LV_USE_NES_FILE_EXPLORER*/
