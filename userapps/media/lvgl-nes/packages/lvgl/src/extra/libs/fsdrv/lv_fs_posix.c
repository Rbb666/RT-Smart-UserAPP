...
static void * fs_open(lv_fs_drv_t * drv, const char * path, lv_fs_mode_t mode)
{
    ...
    LV_LOG_USER("LV_FS_POSIX_PATH:%s", buf);

    int f = open(buf, flags);
    if(f < 0) {
        LV_LOG_ERROR("open error :%d", f);
        return NULL;
    }
...
static void * fs_dir_open(lv_fs_drv_t * drv, const char * path)
{
    ...
    char buf[256];
    sprintf(buf, LV_FS_POSIX_PATH "%s", path);
    LV_LOG_USER("fs_dir_open:path:%s", path);
    LV_LOG_USER("fs_dir_open:buf:%s", buf);
    return opendir(buf);
#else
...
static lv_fs_res_t fs_dir_read(lv_fs_drv_t * drv, void * dir_p, char * fn)
{
    ...
    do {
        entry = readdir(dir_p);
        if(entry) {
            if(entry->d_type == 4) sprintf(fn, "/%s", entry->d_name);
